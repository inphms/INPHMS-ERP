...
from inphms.tools.floatutils import float_round
from inphms.tools import _, html_sanitize, hmac
...
from inphms.tools.urls import urljoin as url_join
from inphms.tools.stringutils import format_decimalized_amount
...
class DigestDigest(models.Model):
    ...
    def _get_unsubscribe_token(self, user_id):
        ...
        """
        return hmac(self.env(su=True), 'digest-unsubscribe', (self.id, user_id))
        ...
        for col_index, (tf_name, tf) in enumerate(self._compute_timeframes(company)):
            ...
            for index, field_name in enumerate(digest_fields):
                ...
                if self._fields['%s_value' % field_name].type == 'monetary':
                    converted_amount = format_decimalized_amount(compute_value)
    ...
    def _compute_kpis_actions(self, company, user):
        ...
        :returns: key: kpi name (field name), value: an action that will be
          concatenated with /inphms/action-{action}
        :rtype: dict
        """
        return {}

...
from inphms.addons.mail.controllers.thread import ThreadController
...
class PortalChatter(ThreadController):
    ...
    @route('/mail/avatar/mail.message/<int:res_id>/author_avatar/<int:width>x<int:height>', type='http', auth='public')
    ...
    @route("/portal/chatter_init", type="jsonrpc", auth="public", website=True)
    ...
    @route('/mail/chatter_fetch', type='jsonrpc', auth='public', website=True)
    ...
    @route(['/mail/update_is_internal'], type='jsonrpc', auth="user", website=True)

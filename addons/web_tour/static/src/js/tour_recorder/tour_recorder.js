...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasInphmsClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasInphmsClass;
        ...
        const inphmsClass = [...currentElem.classList].find((c) => c.match(INPHMS_CLASS_REGEX));
        if (inphmsClass) {
            currentPredicate = `.${inphmsClass}`;
            hasInphmsClass = true;
        }

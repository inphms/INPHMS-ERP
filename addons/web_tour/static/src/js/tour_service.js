import { Component, markup, whenReady, validate } from "@inphms/owl";
    ...
    start: async (env, { orm, effect, overlay }) => {
        ...
        async function resumeTour() {
            ...
            if (tourConfig.mode === "auto") {
                if (!inphms.loader.modules.get("@web_tour/js/tour_automatic/tour_automatic")) {
                    await loadBundle("web_tour.automatic", { css: false });
                }
                const { TourAutomatic } = inphms.loader.modules.get(
                    "@web_tour/js/tour_automatic/tour_automatic"
                );
                new TourAutomatic(tour).start();
            } else {
                await loadBundle("web_tour.interactive");
                const { TourPointer } = inphms.loader.modules.get(
                ...
                const { TourInteractive } = inphms.loader.modules.get(
        ...
        async function tourRecorder() {
            ...
            const { TourRecorder } = inphms.loader.modules.get(
        ...
        inphms.startTour = startTour;
        inphms.isTourReady = (tourName) => getTourFromRegistry(tourName).wait_for.then(() => true);

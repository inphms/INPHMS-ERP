...
import { click, edit, keyDown, keyUp, press, queryOne } from "@inphms/hoot-dom";
import { animationFrame, runAllTimers } from "@inphms/hoot-mock";
import {
    contains,
    defineWebModels,
    mountWithCleanup,
...
test("Click on element with unique inphms class", async () => {
...
test("Click on element with no unique inphms class", async () => {
    await mountWithCleanup(
        `
        <div class="o_parent">
            <div class="o_child_1 click"></div>
...
test("Find the nearest inphms class", async () => {

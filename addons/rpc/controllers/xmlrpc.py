...
from inphms.server import dispatch_rpc, Controller, route, Response
from inphms.exceptions import RedirectWarning, MissingError, AccessError, AccessDenied, UserError
from inphms.tools import exception_to_unicode, frozendict, lazy
from inphms.orm.fields import Date, Datetime, Command
...
# User code must use the exceptions defined in ``inphms.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, RedirectWarning):
...
def xmlrpc_handle_exception_string(e):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(exception_to_unicode(e), formatted_info)

    return dumps(fault)


class InphmsMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, inphms is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = InphmsMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}

...
from ...mail.tools.discuss import Store
from inphms.tools import convertutils, _, email_normalize, email_normalize_all, \
    format_time, float_is_zero
from inphms.tools.intervals import Intervals
from inphms.orm.fields import Domain
from inphms.exceptions import ValidationError, AccessError, RedirectWarning, UserError
...
class HrEmployee(models.Model):
    ...
    def action_create_users(self):
        ...
        employee_emails = [
            normalized_email
            for employee in self
            for normalized_email in email_normalize_all(employee.work_email)
        ]
        ...
        for employee in self:
            ...
            if not employee.work_email:
                ...
                continue
            if not email_normalize(employee.work_email):
    ...
    def _check_access(self, operation):
        ...
        # Since Inphms 19, one must have read access to the comodel to modify the
        # relation.
    ...
    def _load_demo_data(self):
        ...
        convertutils.convert_file(env=self.sudo().env, module='hr', filename='data/scenarios/hr_scenario.xml', idref=None, mode='init')
        if 'resume_line_ids' in self:
            convertutils.convert_file(env=self.env, module='hr_skills', filename='data/scenarios/hr_skills_scenario.xml', idref=None, mode='init')
        return action_reload
    ...
    def _load_scenario(self):
        ...
        if demo_tag:
            return
        convertutils.convert_file(self.env, 'hr', 'data/scenarios/hr_scenario.xml', None, mode='init')

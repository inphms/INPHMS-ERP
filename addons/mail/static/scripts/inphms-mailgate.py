...
# inphms-mailgate
#
# This program will read an email from stdin and forward it to inphms. Configure
# a pipe alias in your mail server to use it, postfix uses a syntax that looks
# like:
#
# email@address: "|/home/inphms/src/inphms-mail.py"
#
# while exim uses a syntax that looks like:
#
# *: |/home/inphms/src/inphms-mail.py
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="Inphms database name (default: %default)", default='inphms')
    op.add_option("-u", "--userid", dest="userid", help="Inphms user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="Inphms user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="Inphms host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="Inphms port (default: %default)", default=8069, type=int)
    ...
    try:
        ...
        if e.faultString == 'Access denied' and e.faultCode == 3:
            ...
            postfix_exit(EX_NOUSER, "alias does not exist in inphms\n", o.debug)
        else:
...
try:
    ...
    # may include the password for the inphms user.

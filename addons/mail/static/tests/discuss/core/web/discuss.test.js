...
import { describe, test } from "@inphms/hoot";
import {
    asyncStep,
    Command,
    onRpc,
...
test("Auto-open InphmsBot chat when opening discuss for the first time", async () => {
    // Inphmsbot chat has onboarding for using Discuss app.
    // We assume pinned inphmsbot chat without any message seen means user just started using Discuss app.
    const pyEnv = await startServer();
    ...
    await start();
    await openDiscuss();
    await contains(".o-mail-DiscussContent-threadName", { value: "InphmsBot" });
});

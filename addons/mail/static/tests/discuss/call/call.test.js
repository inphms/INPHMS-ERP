...
import { advanceTime, hover, manuallyDispatchProgrammaticEvent, queryFirst } from "@inphms/hoot-dom";
import { mockSendBeacon, mockUserAgent } from "@inphms/hoot-mock";
import {
    asyncStep,
    Command,
    mockService,
...
test("no call with inphmsbot", async () => {
    const pyEnv = await startServer();

...
export class OutOfFocusService {
    ...
    async notify(message, thread) {
        ...
        let icon = "/mail/static/src/img/inphmsbot_transparent.png";
        if (!author) {
    ...
    async sendNotification({ message, sound = true, title, type, icon }) {
        ...
        try {
            ...
            if (error.message.includes("ServiceWorkerRegistration")) {
                this.sendInphmsNotification(message, { sound, title, type });
            } else {
                throw error;
            }
        }
     ...
     */
    async sendInphmsNotification(message, options) {
        const { sound } = options;

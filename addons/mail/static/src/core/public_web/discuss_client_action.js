...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@inphms/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const inphmsbotChat = this.store.inphmsbot?.searchChat();
            const selfMember = inphmsbotChat?.self_member_id;
            if (inphmsbotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                inphmsbotChat.setAsDiscussThread(false);
            }
            return;
        }

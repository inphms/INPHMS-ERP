 ...
 * @param {Object} param1
 * @param {string} [param1.action] inphms client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/inphms/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/inphms/\\d+/action-${action}`),
            new RegExp(`/inphms/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/inphms/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/inphms/${modelPath}/${res_id}`));
        }
    }
});

...
from inphms.tools import _, email_normalize, parse_contact_from_email, formataddr, \
    unique, format_datetime
from inphms.tools.mailutils import email_split_and_format, email_split_and_format_normalize
from ..tools.alias_error import AliasError
...
class Base(models.AbstractModel):
    ...
    def _mail_get_partners(self, introspect_fields=False):
        ...
        records_partners = {}
        for record in self:
            pids = unique(pid for fn in partner_fields for pid in record[fn].ids)
    ...
    def _message_add_default_recipients(self):
        ...
        for record in self:
            ...
            if cc_fn:
                email_cc_lst = email_split_and_format_normalize(record[cc_fn]) or [record[cc_fn]]
    ...
    def _message_get_default_recipients(self, with_cc=False, all_tos=False):
        ...
        # ban emails: never propose inphmsbot nor aliases
        all_emails = []
        ...
        for record in self:
            ...
            # if emails match partners, use partners to have more information
                ...
                email_normalize(email) in recipients.mapped('email_normalized') for email in email_to_lst
            ):
                partner_ids = recipients.ids
                email_to = ''
            else:
    ...
    def _message_add_suggested_recipients(self, force_primary_email=False):
        ...
        for record in self:
            if force_primary_email:
                suggested[record.id]['email_to_lst'] += email_split_and_format_normalize(force_primary_email)
            else:
    ...
    def _message_get_suggested_recipients_batch(self, reply_discussion=False, reply_message=None,
        ...
        for record in self:
            followers = record.message_partner_ids if is_mail_thread else record.env['res.partner']
            partners = self.env['res.partner'].browse(unique(
    ...
    def _notify_get_reply_to_formatted_email(self, record_email, author_id=False):
        ...
        formatted_email = formataddr((author_name, record_email))
        if len(formatted_email) > length_limit:
            formatted_email = formataddr((self.env.user.name, record_email))
        if len(formatted_email) > length_limit:
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if not self:
            return headers
        self.ensure_one()
        headers['X-Inphms-Objects'] = f"{self._name}-{self.id}"
    ...
    def _find_value_from_field_path(self, field_path):
        ...
        if any(isinstance(value, datetime) for value in field_value):
            ...
            return ' '.join([f"{format_datetime(self.env, value, tz=tz)} {tz}"

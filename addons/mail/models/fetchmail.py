...
from inphms.tools import exception_to_unicode, _
...
class InphmsIMAP4_SSL(InphmsIMAP4, IMAP4_SSL):
    pass


class InphmsPOP3(POP3):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._unread_messages = None
...
class InphmsPOP3_SSL(InphmsPOP3, POP3_SSL):
    pass
...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/inphms-mailgate.py')
...
inphms-mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
Example configuration for the postfix mta running locally:
/etc/postfix/virtual_aliases: @youdomain inphms_mailgate@localhost
/etc/aliases:
inphms_mailgate: "|/path/to/inphms-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = InphmsIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else InphmsIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = InphmsPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else InphmsPOP3(server, port, timeout=MAIL_TIMEOUT)
    ...
    def button_confirm_login(self):
        for server in self:
            ...
            try:
                ...
                raise UserError(_("Connection test failed: %s", exception_to_unicode(err)))
            finally:
                try:
                    if connection:

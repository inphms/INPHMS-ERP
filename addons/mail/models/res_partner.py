...
from inphms.tools import parse_contact_from_email, limited_field_access_token, _
...
class ResPartner(models.Model):
    ...
    def _compute_im_status(self):
        ...
        inphmsbot_id = self.env['ir.model.data']._xmlid_to_res_id('base.partner_root')
        inphmsbot = self.env['res.partner'].browse(inphmsbot_id)
        if inphmsbot in self:
            inphmsbot.im_status = 'bot'
    ...
    def _find_or_create_from_emails(self, emails, ban_emails=None,
        ...
        name_emails = [parse_contact_from_email(email) for email in emails]

...
from inphms.tools import formataddr, email_normalize
from inphms.tools.mailutils import email_split_tuples, email_split_and_format_normalize
...
class MailThreadCc(models.AbstractModel):
    ...
    def _mail_cc_sanitized_raw_dict(self, cc_string):
        ...
        if not cc_string:
            return {}
        return {
            email_normalize(email): formataddr((name, email_normalize(email)))
            for (name, email) in email_split_tuples(cc_string)
        }
    ...
    def _message_add_suggested_recipients(self, force_primary_email=False):
        ...
        for record in self.filtered('email_cc'):
            suggested[record.id]['email_to_lst'] += email_split_and_format_normalize(record.email_cc)
        return suggested

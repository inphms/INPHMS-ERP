...
from inphms.modules import Registry, SUPERUSER_ID, current_test, Environment
from inphms.addons.base.models.ir_mail_server import MailDeliveryException
from inphms.exceptions import UserError, ValidationError
from inphms.tools import exception_to_unicode, _, split_every, html2plaintext, is_html_empty
from inphms.tools.mailutils import email_normalize, email_anonymize, email_split_and_format_normalize, append_content_to_html, \
    email_normalize_all, formataddr
...
class MailMail(models.Model):
    ...
    auto_delete = fields.Boolean(
        'Auto Delete',
        help="This option permanently removes any track of email after it's been sent, including from the Technical menu in the Settings, in order to preserve storage space of your Inphms database.")
    ...
    def _prepare_outgoing_list(self, mail_server=False, doc_to_followers=None):
        ...
        email_list = []
        if self.email_to:
            email_to_normalized = email_normalize_all(self.email_to)
            email_to = email_split_and_format_normalize(self.email_to)
            email_list.append({
                'email_cc': [],
                'email_to': email_to,
        ...
        if self.email_cc:
            if email_list:
                email_list[0]['email_cc'] = email_split_and_format_normalize(self.email_cc)
                email_list[0]['email_to_normalized'] += email_normalize_all(self.email_cc)
            else:
                email_list.append({
                    'email_cc':  email_split_and_format_normalize(self.email_cc),
                    'email_to': [],
                    'email_to_normalized': email_normalize_all(self.email_cc),
                    'email_to_raw': False,
                    'partner_id': False,
                })
        ...
        for partner in self.recipient_ids:
            ...
            email_to_normalized = email_normalize_all(partner.email)
            email_to = [
                formataddr((partner.name or "", email or "False"))
        ...
        if url_attachments:
            ...
            body = append_content_to_html(body, attachments_links, plaintext=False)
        ...
        for email_values in email_list:
            ...
            results.append({
                ...
                'body_alternative': html2plaintext(body_personalized),
    ...
    def _split_by_mail_configuration(self):
        ...
        for mail, values in zip(self, mail_values):
            ...
            emails_from = email_split_and_format_normalize(values['email_from'])
        ...
        for (mail_server_id, alias_domain_id, smtp_from), record_ids in group_per_smtp_from.items():
            for batch_ids in split_every(batch_size, record_ids):
    ...
    def send_after_commit(self):
        ...
        def send_emails_with_new_cursor():
            ...
            with db_registry.cursor() as cr:
                env = Environment(cr, SUPERUSER_ID, _context)
        ...
        for mail_server_id, alias_domain_id, smtp_from, batch_ids in self._split_by_mail_configuration():
            ...
            try:
                ...
                if raise_exception:
                    ...
                    # exceptions, it is encapsulated into an Inphms MailDeliveryException
                    raise MailDeliveryException(_('Unable to connect to SMTP Server'), exc)
                else:
                    batch = self.browse(batch_ids)
                    batch.write({'state': 'exception', 'failure_reason': exception_to_unicode(exc)})
    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None, alias_domain_id=False,
        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                # protect against ill-formatted email_from when formataddr was used on an already formatted email
                emails_from = email_split_and_format_normalize(mail.email_from)
                ...
                for email in email_list:
                    ...
                    try:
                        ...
                        if str(error) == IrMailServer.NO_VALID_RECIPIENT:
                            ...
                            failure_reason = exception_to_unicode(error)
                ...
                if res:  # mail has been sent at least once, no major exception occurred
                    ...
                    if not current_test:
                        _logger.info(
                            ...
                            mail.id,
                            mail.message_id,
                            email_normalize(msg['from']),  # FROM should not change, so last msg good enough
                            ', '.join(
                                repr(email_anonymize(email_normalize(m['email_to'])))
                                for m in email_list
                            ),
                        )
                ...
                if not failure_reason:
                    failure_reason = exception_to_unicode(e)
                if not failure_type:
                    failure_type = "unknown"

...
from inphms.tools import _, is_html_empty, html_sanitize
...
class MailComposerMixin(models.AbstractModel):
    ...
    def _compute_body(self):
        ...
        template, reset it. """
        for composer_mixin in self:
            if not is_html_empty(composer_mixin.template_id.body_html):
    ...
    def _compute_body_has_template_value(self):
        ...
        as much as possible. """
        for composer_mixin in self:
            if not is_html_empty(composer_mixin.body) and composer_mixin.template_id:
                ...
                sanitized_template_value = html_sanitize(template_value, **sanitize_vals)

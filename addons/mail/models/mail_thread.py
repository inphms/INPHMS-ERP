...
from inphms.exceptions import MissingError, AccessError, UserError
from inphms.orm.fields import Domain
from inphms.tools import is_html_empty, html_escape, html2plaintext, \
    clean_context, split_every, ormcache, is_list_of, _
from inphms.databases import SQL
from inphms.tools.mailutils import append_content_to_html, decode_message_header, \
    email_normalize, email_normalize_all, email_split, email_split_and_format, \
    email_split_and_format_normalize, email_split_and_normalize, formataddr, html_sanitize, generate_tracking_message_id, \
    unfold_references
from ..tools.discuss import Store
from ..tools.web_push import push_to_end_point, DeviceUnreachableError, \
...
class MailThread(models.AbstractModel):
    ...
    def _check_can_update_message_content(self, messages):
        ...
        if any(message.message_type != 'comment' for message in messages):
            raise UserError(_("Only messages type comment can have their content updated"))
    ...
    def _detect_write_to_catchall(self, msg_dict):
        ...
        # (see inphms/inphms#161782), to clean when reaching master
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            # switch to inphmsbot for all incoming message creation
            ...
            for x in ('from', 'recipients',
                      ...
                      'references', 'in_reply_to', 'x_inphms_message_id',
            ...
            if thread_root._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # checks anyway for real author and filters inactive (like inphmsbot)
    ...
    def _partner_find_from_emails(self, records_emails, avoid_alias=True, ban_emails=None,
        ...
        # inspired notably from inphms/inphms@80a0b45df806ffecfb068b5ef05ae1931d655810; final
    ...
    def message_post(self, *,
        ...
        :param str outgoing_email_to: comma-separated list of emails to notify in
            addition to partner_ids. Experimental support as of Inphms v19;
    ...
    def _message_compute_real_author(self, author_id):
        ...
        # message author. In any case avoid inphmsbot.
        if self.env.user.active:  # note that inphmsbot is always inactive, there is a python check
    ...
    def _notify_by_email_get_base_mail_values(self, message, recipients_data, additional_values=None):
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in Inphms)
    ...
    def _notify_by_web_push_prepare_payload(self, message, msg_vals=False, force_record_name=False):
        ...
        if author_id:
            ...
            icon = '/web/static/img/inphms-icon-192x192.png'

        if is_html_empty(body) and message.attachment_ids:
    ...
    def _notify_get_recipients_classify(self, message, recipients_data,
        ...
        :return: list of groups (see '_notify_get_recipients_groups')
          with 'recipients' key filled with matching partners, like
            [{
                'active': True,
                'button_access': {'url': 'https://inphms.com/url', 'title': 'Title'},
                'has_button_access': False,
    ...
    def _notify_get_recipients_for_extra_notifications(self, message, recipients_data, msg_vals=False):
        """ Never send to author and to people outside Inphms (email) except comments """
        notif_pids = []
        notif_pids_notinbox = []

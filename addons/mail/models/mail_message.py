...
from inphms.tools import clean_context, groupby, OrderedSet, _
from inphms.exceptions import AccessError, MissingError
from inphms.orm.fields import Command, Domain
from inphms.tools.mailutils import html_to_inner_content, is_html_empty, generate_tracking_message_id, email_split_tuples
from inphms.databases import SQL
from inphms.orm import models, fields, api
from inphms.modules.utils import get_module_icon
    ...
    * snailmail.letter (snailmail addon)
    ...
    def _to_store(self, store: Store, fields, *, format_reply=True, msg_vals=False, add_followers=False, followers=None):
        ...
        record_fields = [
            ...
            Store.Attr(
                "module_icon",
                lambda record: get_module_icon(self.env[record._name]._original_module),
        ...
        for message in self:
            ...
            if message.incoming_email_cc:
                data["incoming_email_cc"] = email_split_tuples(message.incoming_email_cc)
            if message.incoming_email_to:
                data["incoming_email_to"] = email_split_tuples(message.incoming_email_to)
            if store.target.is_current_user(self.env):
    ...
    def _is_empty(self):
        self.ensure_one()
        return (
            (not self.body or is_html_empty(self.body))
    ...
    def _get_message_id(self, values):
        if values.get('reply_to_force_new', False) is True:
            message_id = generate_tracking_message_id('reply_to')
        elif self._is_thread_message(vals=values):
            message_id = generate_tracking_message_id('%(res_id)s-%(model)s' % values)
        else:
            message_id = generate_tracking_message_id('private')
        return message_id

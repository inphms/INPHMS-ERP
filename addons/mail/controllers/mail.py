...
from inphms.tools import consteq, file_open, _
from ..tools.discuss import add_guest_to_context
from inphms.server import Response, Controller, route
...
class MailController(Controller):
    ...
    def _redirect_to_messaging(cls):
        url = '/inphms/action-mail.action_discuss'
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        url = f'/inphms/{model_in_url}/{res_id}?{url_encode(url_params, sort=True)}'
        return request.redirect(url)

    @route('/mail/view', type='http', auth='public')
    ...
    @route('/mail/unfollow', type='http', auth='public', csrf=False)
    ...
    @route('/mail/message/<int:message_id>', type='http', auth='public')
    ...
    def export_icon_to_png(self, icon, color='#000', bg=None, size=100, alpha=255, font='/web/static/src/libs/fontawesome/fonts/fontawesome-webfont.ttf', width=None, height=None):
        ...
        if icon.isdigit():
            ...
            if icon in oi_font_char_codes:
                ...
                font = "/web/static/lib/inphms_ui_icons/fonts/inphms_ui_icons.woff"

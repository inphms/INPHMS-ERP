...
from inphms.tools import _, file_open
from inphms.tools.pdf import PdfReadError, extract_page
from inphms.exceptions import UserError, AccessError
from inphms.server import content_disposition, route
...
class AttachmentController(ThreadController):
    ...
    @route("/mail/attachment/upload", methods=["POST"], type="http", auth="public")
    ...
    @route("/mail/attachment/delete", methods=["POST"], type="jsonrpc", auth="public")
    @add_guest_to_context
    ...
    @route(['/mail/attachment/zip'], methods=["POST"], type="http", auth="public")

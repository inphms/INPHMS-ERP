...
from inphms.tools import json_default, OrderedSet
...
def get_notify_payload_max_length(default=8000):
    try:
        length = int(os.environ.get('INPHMS_NOTIFY_PAYLOAD_MAX_LENGTH', default))
    except ValueError:
        _logger.warning("INPHMS_NOTIFY_PAYLOAD_MAX_LENGTH has to be an integer, "
                ...
                with db_connect("postgres").cursor() as cr:
                    for payload in payloads:
                        cr.execute(
                            SQL(
                                "SELECT %s('imbus', %s)",
                                SQL.identifier(INPHMS_NOTIFY_FUNCTION),
                                payload,
                            )
                        )

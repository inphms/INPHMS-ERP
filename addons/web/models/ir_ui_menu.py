    ...
    def load_web_menus(self, debug):
        ...
        for menu in menus.values():
            if not menu['id']:
                ...
                sub_menu_icon = menu['submenu_icon']
                web_icon = menu['web_icon']
                web_icon_data = menu['web_icon_data']
                web_icon_data_white = None
                ...
                if menu['id'] == menu['app_id']:
                    ...
                    if menu.get('web_icon') and webIconlist and backgroundColor is None:
                        iconModule = webIconlist[0]
                        iconUrl = webIconlist[1]
                        if iconUrl and iconUrl.startswith('static'):
                            path, ext = iconUrl.rsplit('.',1)
                            web_icon_white_path = f"{iconModule},{path}_white.{ext}"
                            web_icon_data_white = self._read_image(web_icon_white_path)
                            if web_icon_data_white:
                                web_icon_data_white = web_icon_data_white.decode()
                                web_icon_data_white = re.sub(r'\s/g', "", ('data:%s;base64,%s' % (menu['web_icon_data_mimetype'], web_icon_data_white)))

...
from inphms.tools import OrderedSet, unique, get_lang, dateutils
from inphms.exceptions import AccessError, UserError
from inphms.orm.fields import Domain, Command, NewId, Date
from inphms.orm.models import BaseModel
from inphms.orm.models.utils import READ_GROUP_DISPLAY_FORMAT
from inphms.orm.utils import READ_GROUP_NUMBER_GRANULARITY, READ_GROUP_TIME_GRANULARITY, DomainType, regex_order
from inphms import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT
...
class Base(models.AbstractModel):
    ...
    def _web_read_group_fill_temporal(self, groups, groupby, aggregates, fill_from=False, fill_to=False, min_groups=False):
        ...
        required_dates = list(dateutils.date_range(fill_from, fill_to, interval))
        ...
        if property_type in ('date', 'datetime'):
            ...
            def formatter_property_datetime(value):
                ...
                if func == 'week':
                    ...
                    start = (dateutils.start_of(value, func)).strftime(db_format)
                    end = (dateutils.end_of(value, func) + datetime.timedelta(minutes=1)).strftime(db_format)

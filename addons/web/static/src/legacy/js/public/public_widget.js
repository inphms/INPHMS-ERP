...
function InphmsEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
InphmsEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
InphmsEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Inphms where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new InphmsEvent(this, name, info);

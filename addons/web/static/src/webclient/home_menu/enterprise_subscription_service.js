...
const _t = (str, ...args) => appTranslateFn(str, "web", ...args);
import {ExpirationPanel} from "@web/webclient/home_menu/expiration_panel";
import {cookie} from "@web/core/browser/cookie";
import {rpc} from "@web/core/network/rpc";
const {DateTime} = luxon;
import {Component, reactive, xml} from "@inphms/owl";
...
export class SubscriptionManager {
    constructor(env, {orm, notification}) {
        ...
        if (session.expiration_date) {
            ...
            this.expirationDate = DateTime.utc().plus({
                days: 30
            });
        }
    ...
    async buy() {
        const limitDate = serializeDate(DateTime.utc().minus({
            days: 15
        }));
        const args = [[["share", "=", false], ["login_date", ">=", limitDate], ], ];
        const nbUsers = await this.orm.call("res.users", "search_count", args);
        browser.location = `https://www.inphms.com/inphms-enterprise/upgrade?num_users=${nbUsers}`;
    }
    async submitCode(enterpriseCode) {
        const [oldDate] = await Promise.all([this.orm.call("ir.config_parameter", "get_param", ["database.expiration_date"]), this.orm.call("ir.config_parameter", "set_param", ["database.enterprise_code", enterpriseCode, ]), ]);
        await this.orm.call("publisher_warranty.contract", "update_notification", [[]]);
        const [linkedSubscriptionUrl,linkedEmail,expirationDate] = await Promise.all([this.orm.call("ir.config_parameter", "get_param", ["database.already_linked_subscription_url", ]), this.orm.call("ir.config_parameter", "get_param", ["database.already_linked_email"]), this.orm.call("ir.config_parameter", "get_param", ["database.expiration_date"]), ]);
        if (linkedSubscriptionUrl) {
            ...
            if (this.daysLeft > 30) {
                this.notification.add(_t("Thank you, your registration was successful! Your database is valid until %s.", this.formattedExpirationDate), {
                    type: "success"
                });
            }
        } else {
    ...
    async sendUnlinkEmail() {
        const sendUnlinkInstructionsUrl = await this.orm.call("ir.config_parameter", "get_param", ["database.already_linked_send_mail_url", ]);
    ...
    async renew() {
        const enterpriseCode = await this.orm.call("ir.config_parameter", "get_param", ["database.enterprise_code", ]);
        const url = "https://www.inphms.com/inphms-enterprise/renew";
    ...
    async upsell() {
        ...
        const [enterpriseCode,nbUsers] = await Promise.all([this.orm.call("ir.config_parameter", "get_param", ["database.enterprise_code"]), this.orm.call("res.users", "search_count", [[["share", "=", false], ["login_date", ">=", limitDate], ], ]), ]);
        const url = "https://www.inphms.com/inphms-enterprise/upsell";
        const contractQueryString = enterpriseCode ? `&contract=${enterpriseCode}` : "";
...
export const enterpriseSubscriptionService = {
    ...
    start(env, {orm, notification}) {
        ...
        return reactive(new SubscriptionManager(env,{
            orm,
            notification
        }));
    },

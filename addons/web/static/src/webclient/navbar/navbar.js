...
export function updateIconSections(section) {
    if (section.submenuIcon) {
        section.submenuIcon = markup(section.submenuIcon);
    }
    for (const subsection of section.childrenTree) {
        updateIconSections(subsection);
    }
}
...
export class NavBar extends Component {
    ...
    getMenuItemHref(payload) {
        return `/inphms/${payload.actionPath || "action-" + payload.actionID}`;
    }
    ...
    _updateMenuAppsIcon() {
        ...
        if (!this.isScopedApp) {
            const title = !this.isInApp && this.hasBackgroundAction ? _t("Previous view") : _t("Home menu");

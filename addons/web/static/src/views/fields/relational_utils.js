...
import { highlightText, inphmsmark } from "@web/core/utils/html";
...
export class Many2XAutocomplete extends Component {
    ...
    onQuickCreateError(error, request) {
        if (
            ...
            error.exceptionName === "inphms.exceptions.ValidationError"
        ) {
    ...
    buildRecordSuggestion(request, record) {
        ...
        return {
            ...
            label: label
                ? highlightText(request, inphmsmark(label), "text-primary fw-bold")
                : _t("Unnamed"),

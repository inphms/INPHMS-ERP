...
(function (inphms) {
    ...
    class ModuleLoader {
        /** @type {InphmsModuleLoader["bus"]} */
        bus = new EventTarget();
        /** @type {InphmsModuleLoader["checkErrorProm"]} */
        checkErrorProm = null;
        /** @type {InphmsModuleLoader["factories"]} */
        factories = new Map();
        /** @type {InphmsModuleLoader["failed"]} */
        failed = new Set();
        /** @type {InphmsModuleLoader["jobs"]} */
        jobs = new Set();
        /** @type {InphmsModuleLoader["modules"]} */
        modules = new Map();
        ...
        /** @type {InphmsModuleLoader["addJob"]} */
        addJob(name) {
            this.jobs.add(name);
            this.startModules();
        }

        /** @type {InphmsModuleLoader["define"]} */
        define(name, deps, factory, lazy = false) {
            ...
            this.factories.set(name, {
                deps,
                fn: factory,
                ignoreMissingDeps: globalThis.__inphmsIgnoreMissingDependencies,
            });
            if (!lazy) {
                this.addJob(name);
        ...
        /** @type {InphmsModuleLoader["findErrors"]} */
        ...
        /** @type {InphmsModuleLoader["findJob"]} */
        findJob() {
        ...
        /** @type {InphmsModuleLoader["reportErrors"]} */
        ...
        /** @type {InphmsModuleLoader["startModules"]} */
        startModules() {
            let job;
        ...
        /** @type {InphmsModuleLoader["startModule"]} */
        startModule(name) {
            /** @type {(dependency: string) => InphmsModule} */
            const require = (dependency) => this.modules.get(dependency);
            this.jobs.delete(name);
            const factory = this.factories.get(name);
            /** @type {InphmsModule | null} */
            let module = null;
            try {
    ...
    inphms.define = loader.define.bind(loader);
    inphms.loader = loader;

    if (inphms.debug && !loader.debug) {

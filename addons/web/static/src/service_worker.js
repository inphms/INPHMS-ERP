...
const cacheName = "inphms-sw-cache";
const homepageURL = "/inphms";
...
const extractSessionInfo = (htmlContent) => {
    const match = htmlContent.match(/inphms\.__session_info__\s*=\s*({.*?});/s);
...
const readDataOnCache = async (url) => {
    ...
    // if you come from /inphms to project the url is now /inphms/project, but it doesn't exist in cache so use /inphms instead
    if (!response) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/inphms?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("inphms_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "inphms_share_target_ack",
            });
        })()
    );
};

...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "inphms";
}
 ...
 * external website). Unfortunately, Inphms wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/inphms") &&
            (["/web", "/inphms"].includes(url.pathname) || url.pathname.startsWith("/inphms/")) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/inphms") && url.hash) {

...
/**
 * @typedef {import("../../env").InphmsEnv} InphmsEnv
...
export function swallowAllVisitorErrors(env, error, originalError) {
    if (!user.isInternalUser && !inphms.debug && !session.test_mode) {
        return true;
    }
}
...
if (user.isInternalUser === undefined) {
    ...
    // apparently not be present in all Inphms screens at the moment... TODO ?

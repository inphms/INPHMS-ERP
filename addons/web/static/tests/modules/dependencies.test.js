...
function invalidImportsFrom(folder, allowedFolders) {
    ...
    const modulesToCheck = Array.from(inphms.loader.modules.keys()).filter((module) =>
        module.startsWith(`@web/${folder}/`)
    );
    const invalidDeps = {};
    for (const module of modulesToCheck) {
        const invalid = inphms.loader.factories.get(module).deps.filter((dep) => {
            // owl and @web/session are allowed everywhere
            if (dep === "@inphms/owl" || dep === "@web/session") {
                return false;
            }

...
import { Deferred, animationFrame, mockTimeZone, runAllTimers } from "@inphms/hoot-mock";

import { onWillDestroy, onWillStart, reactive, useState } from "@inphms/owl";
...
test("display correct value after validation error", async () => {
    ...
    function validationHandler(env, error, originalError) {
        if (originalError.data.name === "inphms.exceptions.ValidationError") {
            return true;
        }
    }
...
test("one2many form view with action button", async () => {
    ...
    // See https://github.com/inphmsinphms/issues/24189
    mockService("action", {
        doActionButton(params) {

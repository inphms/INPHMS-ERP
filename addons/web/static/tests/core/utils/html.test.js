import { htmlEscape, markup } from "@inphms/owl";
...
test("inphmsmark", () => {
    expect(inphmsmark("").toString()).toBe("");
    expect(inphmsmark("**test**").toString()).toBe("<b>test</b>");
    expect(inphmsmark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(inphmsmark("--test--").toString()).toBe("<span class='text-muted'>test</span>");
    expect(inphmsmark("--test-- something else --test--").toString()).toBe(
        "<span class='text-muted'>test</span> something else <span class='text-muted'>test</span>"
    );
    expect(inphmsmark("`test`").toString()).toBe(
    ...
    expect(inphmsmark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(inphmsmark("test\ntest2").toString()).toBe("test<br/>test2");
    expect(inphmsmark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(inphmsmark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});

...
import { describe, test, expect } from "@inphms/hoot";
...
import { click, freezeTime, queryAllTexts } from "@inphms/hoot-dom";
import {
    ClientErrorDialog,
    Error504Dialog,
    ErrorDialog,
...
test("WarningDialog", async () => {
    ...
    await mountWithCleanup(WarningDialog, {
        env,
        props: {
            exceptionName: "inphms.exceptions.UserError",
            message: "...",
...
test("RedirectWarningDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Inphms Warning");
    expect("main").toHaveText("Some strange unreadable message");
...
test("SessionExpiredDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Inphms Session Expired");
    expect("main p").toHaveText(
        "Your Inphms session expired. The current page is about to be refreshed."
    );
    expect(".o_dialog footer button").toHaveText("Close");

import { describe, expect, test, beforeEach } from "@inphms/hoot";
import { queryAllTexts, waitFor } from "@inphms/hoot-dom";
import {
    contains,
    defineActions,
    defineModels,
...
import { mockTouch, runAllTimers } from "@inphms/hoot-mock";
...
test("breadcrumbs are updated when clicking on embeddeds", async () => {
    ...
    expect(browser.location.href).toBe("https://www.hoot.test/inphms/action-1");
    await contains(".o_embedded_actions > button > span:contains('Embedded Action 2')").click();
    await runAllTimers();
    expect(browser.location.href).toBe("https://www.hoot.test/inphms/action-3");
    expect(router.current.action).toBe(3, {
        message: "the current action should be the one of the embedded action previously clicked",
    });
    ...
    await contains(".o_embedded_actions > button > span:contains('Embedded Action 3')").click();
    await runAllTimers();
    expect(browser.location.href).toBe("https://www.hoot.test/inphms/action-4");
    expect(router.current.action).toBe(4, {
        message: "the current action should be the one of the embedded action previously clicked",
    });

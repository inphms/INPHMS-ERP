import { beforeEach, destroy, expect, test } from "@inphms/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@inphms/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@inphms/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@inphms/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/inphms/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/inphms/my-path");
});

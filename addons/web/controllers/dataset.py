...
from inphms.server.utils import thread_local, request
from inphms.service.model import call_kw
...
class DataSet(Controller):
    ...
    @route(['/web/dataset/call_kw', '/web/dataset/call_kw/<path:path>'], type='jsonrpc', auth="user", readonly=_call_kw_readonly)
    def call_kw(self, model, method, args, kwargs, path=None):
        if path != f'{model}.{method}':
            thread_local.rpc_model_method = f'{model}.{method}'
        return call_kw(request.env[model], method, args, kwargs)

    @route(['/web/dataset/call_button', '/web/dataset/call_button/<path:path>'], type='jsonrpc', auth="user", readonly=_call_kw_readonly)

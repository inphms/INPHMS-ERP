...
from inphms.tools import file_open, file_path, image_process
from inphms.modules import Manifest


class WebManifest(Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/inphms?menu_id=%s' % data.mapped('res_id')[0],
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'Inphms')
        manifest = {
            'name': web_app_name,
            'scope': '/inphms',
            'start_url': '/inphms',
        ...
        manifest['icons'] = [{
            'src': '/web/static/img/inphms-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    @route('/web/manifest.webmanifest', type='http', auth='public', methods=['GET'], readonly=True)
    ...
    @route('/web/service-worker.js', type='http', auth='public', methods=['GET'], readonly=True)
    def service_worker(self):
        response = request.make_response(
            self._get_service_worker_content(),
            [
                ('Content-Type', 'text/javascript'),
                ('Service-Worker-Allowed', '/inphms'),
            ]
        )
        return response

    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/inphms')
        """
    ...
    def _icon_path(self):
        return 'web/static/img/inphms-icon-192x192.png'

    @route('/inphms/offline', type='http', auth='public', methods=['GET'], readonly=True)
    def offline(self):
        """ Returns the offline page delivered by the service worker """
        return request.render('web.webclient_offline', {
            'inphms_icon': base64.b64encode(file_open(self._icon_path(), 'rb').read())
        })

    @route('/scoped_app', type='http', auth='public', methods=['GET'])
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/inphms-icon-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    @route('/scoped_app_icon_png', type='http', auth='public', methods=['GET'])
    def scoped_app_icon_png(self, app_id, add_padding=False):
        ...
        if app_icon['type'] == "image/svg+xml":
            ...
            manifest = Manifest.for_addon(app_id, display_warning=False)
            add_padding = True
    ...
    @route('/web/manifest.scoped_app_manifest', type='http', auth='public', methods=['GET'])
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /inphms)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {

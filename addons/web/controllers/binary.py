...
try:
    ...
    from inphms.tools._vendor.send_file import send_file

from inphms.server import Controller, route, Response
from inphms.server.streaming import Stream
from inphms.server.utils import request, STATIC_CACHE_LONG
from inphms import config
from inphms.tools import replace_exceptions, str2bool, file_path, guess_mimetype, file_open, _
from inphms.tools.imageutils import image_guess_size_from_field_name
from inphms.exceptions import UserError, AccessError
from inphms.addons.base.models.assetsbundle.utils import ANY_UNIQUE
from inphms.modules import SUPERUSER_ID, Environment
...
Inphms is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Inphms is receiving the
request.
...
class Binary(Controller):
    ...
    def content_assets(self, filename=None, unique=ANY_UNIQUE, nocache=False, assets_params=None):
        ...
        if not attachment:
            ...
            with cursor_manager as rw_cr:
                rw_env = Environment(rw_cr, env.user.id, {})
                try:
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Inphms logo instead", exc_info=True)

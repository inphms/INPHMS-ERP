...
from inphms.server import route, Controller, content_disposition
from inphms.server.utils import request
from inphms.tools import osutils, _


class TableExporter(Controller):

    @route('/web/pivot/export_xlsx', type='http', auth="user", readonly=True)
    def export_xlsx(self, data, **kw):
        ...
        filename = osutils.clean_filename(_("Pivot %(title)s (%(model_name)s)", title=jdata['title'], model_name=jdata['model']))

...
from inphms.server.utils import request, db_filter
...
class Session(Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Inphms Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        with ExitStack() as stack:
            ...
            if auth_info['uid'] != request.session.uid:
                # Crapy workaround for unupdatable Inphms Mobile App iOS (Thanks Apple :@) and Android
    ...
    @route('/web/session/get_lang_list', type='jsonrpc', auth="none")
    ...
    def account(self):
        ...
        return 'https://accounts.inphms.com/oauth2/auth?' + url_encode(params)

    @route('/web/session/destroy', type='jsonrpc', auth='user', readonly=True)
    def destroy(self):
        request.session.logout()

    @route('/web/session/logout', type='http', auth='none', readonly=True)

...
from inphms.server.utils import request, STATIC_CACHE_LONG, db_list
from inphms.service import security
from .utils import is_user_internal, ensure_db, _get_login_redirect_url
from inphms.databases import db_connect
from inphms.tools import hmac
from inphms.exceptions import SessionExpiredException, AccessError, AccessDenied
from inphms.tools.translate import LazyTranslate, _
...
class Home(Controller):
    ...
    @route(['/web', '/inphms', '/inphms/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    ...
    @route('/web/webclient/load_menus', type='http', auth='user', methods=['GET'], readonly=True)
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                db_connect('postgres').cursor().close()

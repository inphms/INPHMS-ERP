...
from inphms.server.utils import request, db_filter, db_list, get_default_session
from inphms.server import http
from inphms.tools import file_open
from inphms.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT
...
def ensure_db(redirect='/web/database/selector', db=None):
    ...
    # Ensure db is legit
    if db and db not in db_filter([db]):
        db = None
    ...
    if not db and request.session.db and db_filter([request.session.db]):
        db = request.session.db

    # if no database provided and no database in session, use monodb
    if not db:
        all_dbs = db_list(force=True)
        if len(all_dbs) == 1:
            db = all_dbs[0]
...
def _get_login_redirect_url(uid, redirect=None):
    ...
    if request.session.uid:  # fully logged
        return redirect or ('/inphms' if is_user_internal(request.session.uid)
...
def get_action_triples(env, path, *, start_pos=0):
    """
    Extract the triples (active_id, action, record_id) from a "/inphms"-like path.

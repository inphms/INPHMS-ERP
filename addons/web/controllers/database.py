...
from inphms.server import Controller, route, content_disposition, dispatch_rpc, Response
from inphms.server.utils import request, db_list
from inphms.service import db
from inphms.config import config
from inphms.tools import str2bool, file_open
...
class Database(Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = config.validate_master_password('admin')
        d['list_db'] = config['db_manager']
        d['langs'] = db.exp_list_lang()
        d['countries'] = db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = db_list()
            d['incompatible_databases'] = db.list_db_incompatible(d['databases'])
        except AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = config.validate_master_password('admin')
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            if not re.match(DBNAME_PATTERN, name):
                raise Exception('Houston, we have a database naming issue! Make sure you only use letters, numbers, underscores, hyphens, or dots in the database name, and you\'ll be golden.')
            ...
            with Registry(name).cursor() as cr:
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        try:
            ...
            dump_stream = db.dump_db(name, None, backup_format, filestore)

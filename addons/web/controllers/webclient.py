...
from inphms.tools import file_path
from .utils import _local_web_translations


class WebClient(Controller):

    @route('/web/webclient/bootstrap_translations', type='jsonrpc', auth="none")
    ...
    @route('/web/webclient/translations', type='http', auth='public', cors='*', readonly=True)
    def translations(self, hash=None, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = request.env.registry._init_modules.union(config['server_wide_modules'])
    ...
    @route('/web/webclient/version_info', type='jsonrpc', auth="none")
    def version_info(self):
        return inphms.service.common.exp_version()

    @route('/web/tests', type='http', auth='user', readonly=True)
    ...
    @route('/web/tests/legacy', type='http', auth='user', readonly=True)
    def test_suite(self, mod=None, **kwargs):
        return request.render('web.qunit_suite', {'session_info': {'view_info': request.env['ir.ui.view'].get_view_info()}})

    @route('/web/bundle/<string:bundle_name>', auth='public', methods=['GET'], readonly=True)

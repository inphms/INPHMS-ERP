...
from inphms.tools.imageutils import image_data_uri
from inphms.orm.fields import Domain
from inphms.tools import py_to_js_locale, _, float_round
from inphms.service.common import exp_version

import datetime

class HrAttendance(Controller):
    @staticmethod
    ...
    @route('/hr_attendance/kiosk_mode_menu/<int:company_id>', auth='user', type='http')
    ...
    @route('/hr_attendance/get_employees_without_badge', type='jsonrpc', auth='public')
    ...
    @route('/hr_attendance/set_badge', type='jsonrpc', auth='public')
    ...
    @route('/hr_attendance/create_employee', type='jsonrpc', auth='public')
    ...
    @route('/hr_attendance/kiosk_keepalive', auth='user', type='jsonrpc')
    ...
    def open_kiosk_mode(self, token, from_trial_mode=False):
        ...
        if not company:
            ...
            # If current user accesing the kiosk has password in their user account.
            has_password = self.has_password()
            if not from_trial_mode and has_password:
                # if not from trial mode and user has password, do session logout, but keep everything else
                request.session.logout(keep_db=True)
            if (from_trial_mode or (not has_password and not request.env.user.is_public)):
                # if from trial mode, or not user password and user is not public
                kiosk_mode = "settings"
            else:
    ...
    @route('/hr_attendance/attendance_employee_data', type="jsonrpc", auth="public")
    ...
    @route('/hr_attendance/attendance_barcode_scanned', type="jsonrpc", auth="public")
    ...
    @route('/hr_attendance/manual_selection', type="jsonrpc", auth="public")
    ...
    @route('/hr_attendance/attendance_user_data', type="jsonrpc", auth="user", readonly=True)
    ...
    @route('/hr_attendance/set_settings', type="jsonrpc", auth="public")

...
describe("Link creation", () => {
    describe("Creation by space", () => {
        ...
        test("typing invalid URL + space should not convert to link", async () => {
            const { editor, el } = await setupEditor("<p>[]</p>");
            await insertText(editor, "www.inphms");
            await insertText(editor, " ");
            expect(cleanLinkArtifacts(getContent(el))).toBe("<p>www.inphms []</p>");
        });
    });
...
describe("shortcut", () => {
    ...
    test("should be able to create link with ctrl+k and ctrl+k", async () => {
        const { el } = await setupEditor(`<p>[]<br></p>`);
        // Open inphms global command bar
        await press(["ctrl", "k"]);
        await waitFor('.o_command span[title="Create link"]');
...
describe("link preview", () => {
    test("test internal link preview", async () => {
        ...
        onRpc("/inphms/project/1/tasks/8", () => "", { pure: true });
        const { editor, el } = await setupEditor(`<p>[]<br></p>`, {
            config: {
        ...
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/inphms/project/1/tasks/8"
        );
        await animationFrame();
    ...
    test("test external link preview", async () => {
        onRpc("/html_editor/link_preview_external", () => ({
            og_description:
                "From ERP to CRM, eCommerce and CMS. Download Inphms or use it in the cloud. Grow Your Business.",
            og_image: "https://www.inphms.com/web/image/41207129-1abe7a15/homepage-seo.png",
            og_title: "Open Source ERP and CRM | Inphms",
            og_type: "website",
            og_site_name: "Inphms",
            source_url: "http://inphms.com/",
        }));
        const { editor } = await setupEditor(`<p>[]<br></p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill("http://inphms.com/");
        await animationFrame();
        expect(".o_we_replace_title_btn").toHaveCount(1);
        expect(".o_extra_info_card").toHaveCount(1);
        expect(".o_we_url_link").toHaveText("Open Source ERP and CRM | Inphms");
    ...
    test("test internal metadata cached correctly", async () => {
        ...
        onRpc("/inphms/cachetest/8", () => "", { pure: true });
        const { editor } = await setupEditor(`<p>abc[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/inphms/cachetest/8"
        );
        await animationFrame();
    ...
    test("should use cached metadata even if protocol changes", async () => {
        ...
        const testUrl = `${wrongProtocol}//${window.location.host}/inphms/cachetest/8`;
    ...
    test("test external metadata cached correctly", async () => {
        const title = "Open Source ERP and CRM | Inphms";
        const description =
            "From ERP to CRM, eCommerce and CMS. Download Inphms or use it in the cloud. Grow Your Business.";
        onRpc("/html_editor/link_preview_external", () => {
...
describe("label is a valid URL", () => {
    ...
    test("popover should display href URL even if label is a valid URL and differs from href", async () => {
        await setupEditor('<p><a href="https://inphms.com/">googl[]e.com</a></p>');
        await waitFor(".o-we-linkpopover", { timeout: 1500 });
        expect(queryFirst(".o-we-linkpopover a").href).toBe("https://inphms.com/");
        await click(".o_we_edit_link");
        await waitFor(".o_we_href_input_link");
        expect("input.o_we_href_input_link").toHaveValue("https://inphms.com/");
    });
});

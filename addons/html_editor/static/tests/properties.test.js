import { expect, test } from "@inphms/hoot";
import {
    defineModels,
    fields,
    models,
...
import { queryOne } from "@inphms/hoot-dom";
...
test("properties: html", async () => {
    ...
    expect(`[name="properties"] .inphms-editor-editable`).toHaveCount(1);
    expect(`[name="properties"] .inphms-editor-editable .o-paragraph`).toHaveInnerHTML(
        "<b> test </b>"
    );

    setSelection({
        anchorNode: queryOne(`[name="properties"] .inphms-editor-editable .o-paragraph b`),
        anchorOffset: 0,
    });
...
test("properties: html migration", async () => {
    ...
    expect(`[name="properties"] .inphms-editor-editable`).toHaveCount(1);
    expect(`[name="properties"] .inphms-editor-editable a[href*="excalidraw.com"]`).toHaveCount(1);

...
import { beforeEach, describe, expect, getFixture, test } from "@inphms/hoot";
import { click, queryFirst } from "@inphms/hoot-dom";
    ...
    test("Can mount and destroy recursive embedded components in any order", async () => {
        ...
        expect(
            [...fixture.querySelectorAll("[data-embedded]")].filter(
                (elem) => !elem.closest(".inphms-editor-editable")
            )
        ).toEqual([]);
    });

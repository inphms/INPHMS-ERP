...
export class CollaborationSelectionPlugin extends Plugin {
    ...
    static dependencies = ["history", "collaborationInphms", "position", "localOverlay"];
    resources = {
        /** Handlers */
    ...
    /**
     * @param {import("./collaboration_inphms_plugin").CollaborationSelection} selection
     */
    ...
    drawPeerSelection({ selection, peerId }) {
        const peerMetadata = this.dependencies.collaborationInphms.getPeerMetadata(peerId);
        if (!peerMetadata) {
            return;
        }

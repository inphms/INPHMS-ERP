...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        ev.clipboardData.setData("application/vnd.inphms.inphms-editor", htmlContent);
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteInphmsEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteInphmsEditorHtml(clipboardData) {
        const inphmsEditorHtml = clipboardData.getData("application/vnd.inphms.inphms-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (inphmsEditorHtml) {
            const fragment = parseHTML(this.document, inphmsEditorHtml);
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.inphms.inphms-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.inphms.inphms-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&

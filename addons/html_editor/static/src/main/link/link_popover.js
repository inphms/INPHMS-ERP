...
import { Component, useState, useRef, useEffect, useExternalListener } from "@inphms/owl";
...
export class LinkPopover extends Component {
    ...
    async loadAsyncLinkPreview() {
        ...
        if (!protocol.startsWith("http")) {
            ...
            !new RegExp(`^https?://${session.db}\\.inphms\\.com(/.*)?$`).test(url.origin)
        ) {
    ...
    isAbsoluteURLInCurrentDomain() {
        ...
        // its Inphms instance domain. We just suppose it should be a relative
        // URL (if unexpected behavior, the user can just not enter its Inphms
        // instance domain but its real domain, or opt-out from the domain
        // stripping). Mentioning an .inphms.com domain, especially its own
        // one, is always a bad practice anyway.
        return (
            urlObj.origin === window.location.origin ||
            new RegExp(`^https?://${session.db}\\.inphms\\.com(/.*)?$`).test(urlObj.origin)
        );
    }
}

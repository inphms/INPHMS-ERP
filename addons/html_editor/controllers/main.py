...
from ..tools import get_video_url_data
from inphms.exceptions import UserError, MissingError, AccessError
from inphms.tools import file_open, guess_mimetype, _, html_escape
from inphms.tools.imageutils import image_process, image_data_uri, binary_to_image, get_webp_size
# from inphms.addons.iap.tools import iap_tools
from inphms.addons.mail.tools import link_preview
...
DEFAULT_LIBRARY_ENDPOINT = 'https://media-api.inphms.com'
DEFAULT_OLG_ENDPOINT = 'https://olg.api.inphms.com'
...
class HTML_Editor(Controller):
            ...
            if colorMatch:
                ...
                if not re.match(r'(?i)^%s$|^%s$' % (regex_hex, regex_rgba), css_color_value.replace(' ', '')):
                    if re.match('^o-color-([1-5])$', css_color_value):
                        ...
                        raise werkzeug.exceptions.BadRequest()
                user_colors.append([html_escape(css_color_value), colorMatch.group(1)])
            else:
    ...
    def remove(self, ids, **kwargs):
        ...
        for attachment in Attachment.browse(ids):
            ...
            # find them
            url = html_escape(attachment.local_url)
            views = Views.search([
                "|",
                ('arch_db', 'like', '"%s"' % url),
                ('arch_db', 'like', "'%s'" % url)
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if not (
                ...
                and (
                    parsed_preview_url.path.startswith("/inphms")
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `inphms/<model>/<record_id>` so we use `action_name` as model name

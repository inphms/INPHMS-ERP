...
from inphms.modules import Registry, Environment, SUPERUSER_ID, setup_sys_path
from inphms.netsvc import setup_logger
from inphms.tools import topological_sort, unique, profiler
from .common import standalone_tests
import inphms.service.db
import inphms.tests.loader


_logger = logging.getLogger('inphms.tests.test_module_operations')
...
def addons_path(value):
    return config._check_addons_path(config.opts_index['init'], '-i', value)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Inphms modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra Inphms modules")
...
def test_standalone(args):
    ...
    inphms.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        inphms.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'inphms.modules.loading': {'level': 'CRITICAL'},
            'inphms.db': {'level': 'CRITICAL'},
            'inphms.models.unlink': {'level': 'WARNING'},
            'inphms.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })

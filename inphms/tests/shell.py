...
TEST_MODULE_NAME_PATTERN = re.compile(r'^inphms\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if inphms.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in inphms shell')
        return
    ...
    from inphms.server.serving import server  # noqa: PLC0415
    if not server.httpd:

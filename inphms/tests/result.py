...
from . import case
from inphms.databases import sql_counter
...
INPHMS_TEST_MAX_FAILED_TESTS = max(1, int(os.environ.get('INPHMS_TEST_MAX_FAILED_TESTS', sys.maxsize)))

stats_logger = logging.getLogger('inphms.tests.stats')
...
class InphmsTestResult(object):
    ...
    def _checkShouldStop(self):
        if self.total_errors_count() >= INPHMS_TEST_MAX_FAILED_TESTS:
            ...
            if not global_report.shouldStop:
                _logger.error(
                    ...
                    "Remaining tests will be skipped.", INPHMS_TEST_MAX_FAILED_TESTS)
    ...
    def startTest(self, test):
        ...
        self.queries_start = sql_counter
    ...
    def stopTest(self, test):
        ...
        if stats_logger.isEnabledFor(logging.INFO):
            self.stats[test.id()] = Stat(
                ...
                queries=sql_counter - self.queries_start,
            )
    ...
    def collectStats(self, test_id):
        ...
        self.stats[test_id] += Stat(
            ...
            queries=sql_counter - queries_before,
        )

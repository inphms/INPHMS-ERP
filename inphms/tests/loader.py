...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'inphms.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"inphms.upgrade.{module}",
        f"inphms.addons.{module}.migrations",
        f"inphms.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    """
    config_tags = TagsSelector(config['test_tags'])
    ...
    return InphmsSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = InphmsTestResult(global_report=global_report)
    suite(results)

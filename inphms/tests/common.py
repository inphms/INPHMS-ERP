...
import inphms.server
import inphms.server.serving
import inphms.server.fs
import inphms.addons
import inphms.cli
from inphms import config
from inphms.modules import SUPERUSER_ID, Environment, Registry, DummyRLock
from inphms.databases import Cursor, SQL
from inphms.databases.savepoint import Savepoint
from . import case, test_cursor
from .result import InphmsTestResult
from inphms.addons.base.models import ir_actions_report
from inphms.tools import find_in_path, mute_logger, float_compare, profiler, DotDict, lower_logging, \
    single_email_re
from inphms.exceptions import AccessError
from inphms.orm.fields import Command
from inphms.tools.xml_utils import _validate_xml


_logger = logging.getLogger(__name__)

if inphms.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
# The inphms library is supposed already configured.
HOST = '127.0.0.1'
...
def get_db_name():
    ...
    if len(dbnames) > 1:
        sys.exit("-d/--database/db_list has multiple database, please provide a single one")
    return dbnames[0]
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = inphms.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Inphms module name.
    """
    def register(func):
        # register func by inphms module name
        if func.__module__.startswith('inphms.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most inphms operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for Inphms-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('inphms.addons.'):
            if getattr(cls, 'test_tags', None) is None:
                cls.test_tags = {'standard', 'at_install'}
            cls.test_module = cls.__module__.split('.')[2]

    longMessage = True      # more verbose error message by default: https://www.inphms.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('INPHMS_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def run(self, result: InphmsTestResult) -> None:
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~inphms.orm.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(inphms.server.utils.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            inphms.server.utils._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = inphms.server.utils._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('inphms.databases.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/inphms/addons/" in filename:
                        filename = filename.rsplit("/inphms/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: inphms.orm.models.BaseModel,
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case inphms.orm.fields.Many2one():
                        record_value = record_value.id
                    case inphms.orm.fields.One2many() | inphms.orm.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case inphms.orm.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case inphms.orm.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if inphms.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {inphms.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = inphms.server.utils.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if config['test_tags'] and 'is_query_count' in config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | inphms.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(inphms.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = Environment(cr, SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('inphms.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_inphms')

        if scs := config['screencasts']:
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Inphms widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: inphms.server.fs.Session = None
    ...
    @classmethod
    def http_port(cls):
        if inphms.server.serving.server is None:
            return None
        return inphms.server.serving.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('inphms.service.http.request.')]
        ...
        if request_threads:
            ...
            inphms.tools.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        inphms.server.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            inphms.server.http.root.session_store.delete(self.session)

        self.session = session = inphms.server.http.root.session_store.new()
        session.update(inphms.server.utils.get_default_session(), db=get_db_name())
        session.context['lang'] = inphms.server.utils.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('inphms.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from inphms.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from inphms.addons.bus.models.bus import BusBus  # noqa: PLC0415
            ...
            cpu_throttling_os = os.environ.get('INPHMS_BROWSER_CPU_THROTTLING')  # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("INPHMS_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"inphms.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"inphms.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('inphms.server.request.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if config['test_tags'] and 'is_tour' in config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from inphms.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Inphms test suites

...
from inphms.tools import discardattr, frozendict, OrderedSet, LastOrderedSet
from inphms.orm import models, fields
from inphms.databases import TableKind, table_kind as _table_kind, SQL
...
def _add_manual_models(env: Environment):
    ...
    for model_data in env.cr.dictfetchall():
        ...
        table_kind = _table_kind(env.cr, table_name)
        if table_kind not in (TableKind.Regular, None):
            _logger.info(
...
def _setup(model_cls: type[BaseModel], env: Environment):
    ...
    for name, fields_ in definitions.items():
        if model_cls.pool is not None and f'{model_cls._name}.{name}' in model_cls.pool._database_company_dependent_fields:
...
def add_field(model_cls: type[BaseModel], name: str, field: Field):
    ...
    is_class_field = any(
        ...
        for model in [model_cls] + [model_cls.pool[inherit] for inherit in model_cls._inherits if model_cls.pool is not None]
    )

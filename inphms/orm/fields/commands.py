...
class Command(enum.IntEnum):
    """
    :class:`~inphms.fields.One2many` and :class:`~inphms.fields.Many2many` fields
    ...
    def create(cls, values: ValuesType) -> CommandValue:
        ...
        In case of a :class:`~inphms.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~inphms.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int) -> CommandValue:
        ...
        In case of a :class:`~inphms.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int) -> CommandValue:
        ...
        In case of a :class:`~inphms.fields.One2many` relation, the given record
...
if t.TYPE_CHECKING:
    CommandValue = tuple[Command, int, t.Literal[0] | ValuesType | Collection[int]]

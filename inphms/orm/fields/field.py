...
from inphms.exceptions import AccessError, MissingError
from inphms.tools import ReadonlyDict, SENTINEL, Sentinel, unique
from .utils import _global_seq, COMPANY_DEPENDENT_FIELDS, COLLECTION_TYPES, resolve_mro, PYTHON_INEQUALITY_OPERATOR
from ..utils import PREFETCH_MAX, expand_ids, DomainType, determine, SQL_OPERATORS
from inphms.databases import sqlutils, SQL, Query, SQL_ORDER_BY_TYPE
from ..domains import Domain

T = t.TypeVar('T')
if t.TYPE_CHECKING:
    from collections.abc import Collection, Callable, Iterable, Iterator, MutableMapping
    from inphms.orm.models import BaseModel
    from typing import Self
    from inphms.modules import Environment, Registry
    from .utils import IdType
    from ..types import ModelType, ValuesType
...
class Field(t.Generic[T]):
    """ The field descriptor contains the field definition, and manages accesses
        ...
        Example::
            ...
            Field(string: str,
                  ...
                  readonly: bool = False,
                  required: bool = False,
                  index: Literal['btree', 'btree_not_null', 'trigram', None] = None,],
                  default: Callable[[BaseModel], T] | T | None = None,
                  ...
                  aggregator: Literal['count', 'count_distinct', 'bool_and', 'bool_or', 'max', 'min', 'avg', 'sum'] | None = None,
                  group_expand: str | Callable[[BaseModel, ModelType, DomainType], ModelType] | None = None,
    ...
    _args__: dict[str, t.Any] | None = None                                                     # the parameters given to __init__()
    ...
    search: str | Callable[[BaseModel, str, t.Any], DomainType] | None = None                   # search(recs, operator, value) searches on self
    ...
    def _get_attrs(self, model_class: type[BaseModel], name: str) -> dict[str, t.Any]:
    ...
    def _description_falsy_value_label(self, env) -> str | None:
        return self.falsy_value_label if self.falsy_value_label else None # pylint: disable=gettext-variable
    ...
    def update_db(self, model: BaseModel, columns: dict[str, dict[str, t.Any]]) -> bool:
    ...
    # Layer 1
    #
    def update_db_column(self, model: BaseModel, column: dict[str, t.Any]):
        ...
        if not column:
            ...
            sqlutils.create_column(model.env.cr, model._table, self.name, self.column_type[1], self.string)
            return
        if column['udt_name'] == self.column_type[0]:
            return
        if column['is_nullable'] == 'NO':
            sqlutils.drop_not_null(model.env.cr, model._table, self.name)
        self._convert_db_column(model, column)
        column.clear()  # remove information, because it may no longer be valid

    def update_db_notnull(self, model: BaseModel, column: dict[str, t.Any]):
        ...
        if self.required and not has_notnull:
            ...
            def add_not_null():
                ...
                model.pool.post_constraint(
                    model.env.cr,
                    lambda cr: sqlutils.set_not_null(cr, model._table, field.name),
    ...
    # Layer 2
    #
    def _convert_db_column(self, model: BaseModel, column: dict[str, t.Any]):
    ...
    def create(self, record_values: Collection[tuple[BaseModel, t.Any]]) -> None:
    ...
    def write(self, records: BaseModel, value: t.Any) -> None:
    ...
    def _get_cache(self, env: Environment) -> MutableMapping[IdType, t.Any]:
    ...
    def _get_cache_impl(self, env: Environment) -> MutableMapping[IdType, t.Any]:
    ...
    def _filter_not_equal(self, records: ModelType, cache_value: t.Any) -> ModelType:
    ...
    def _update_cache(self, records: BaseModel, cache_value: t.Any, dirty: bool = False) -> None:
    ...
    def recompute(self, records: BaseModel) -> None:
        ...
        def apply_except_missing(func, records: BaseModel):
    ...
    def determine_domain(self, records: BaseModel, operator: str, value) -> t.Any:
...
from .. import models as _models # ruff: noqa

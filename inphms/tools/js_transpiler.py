...
from .dicts import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom inphms modules.
    ...
    legacy_inphms_define = get_aliased_inphms_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_inphms_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_inphms_define:
        content += legacy_inphms_define
    return content
...
def url_to_module_path(url):
    """
    Inphms modules each have a name. (inphms.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_inphms_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the inphms.define call.
    ...
    """
    return f"""inphms.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def relative_path_to_module_path(url, path_rel):
    ...
    result = "/".join(url_split[:-nb_back] + [v for v in path_rel_split if v not in ["..", "."]])
    return url_to_module_path(result)


INPHMS_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @inphms-module                       # '@inphms-module' statement
...
def is_inphms_module(url, content):
    """
    Detect if the file is a native inphms module.
    We look for a comment containing @inphms-module.

    :param url:
    :param content: source code
    :return: is this a inphms module that need transpilation ?
    """
    result = INPHMS_MODULE_RE.match(content)
...
def get_aliased_inphms_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the inphms source base being:
    ...
    > inphms.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @inphms-module */
        // after
        /** @inphms-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @inphms-module */
        // after
        /** @inphms-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = INPHMS_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ninphms.define(`%s`, ['%s'], function (require) {
                        return require('%s');

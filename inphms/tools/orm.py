...
if t.TYPE_CHECKING:
    ...
    from ._vendor.lru import LRU
    from inphms.orm.models import BaseModel
    C = t.TypeVar("C", bound=Callable)

__all__ = ['ormcache_counter', 'ormcache']

_logger = logging.getLogger(__name__)
_logger_lock = threading.RLock()
_logger_state: t.Literal['wait', 'abort', 'run'] = 'wait'
...
def log_ormcache_stats(sig=None, frame=None):    # noqa: ARG001 (arguments are there for signals)
    ...
    if sig == signal.SIGUSR1:
        threading.Thread(target=_log_ormcache_stats,
                         name="inphms.signal.log_ormcache_stats").start()
    elif sig == signal.SIGUSR2:
        show_size = True
        threading.Thread(target=_log_ormcache_stats,
                         name="inphms.signal.log_ormcache_stats_with_size").start()
...
def get_cache_size(
    ...
    from inphms.orm.models import BaseModel  # noqa: PLC0415

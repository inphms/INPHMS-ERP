...
from ._vendor.safe_eval import safe_eval, pytz, time
...
ConvertMode = t.Literal["init", "update"]
IdRef = dict[str, int | t.Literal[False]]
...
def _eval_xml(self, node, env: Environment):
    if node.tag in ('field','value'):
        ...
        if a_eval := node.get('eval'):
            ...
            try:
                ...
                logging.getLogger('inphms.tools.convert.init').error(
...
class xml_import(object):
    def get_env(self, node, eval_context=None) -> Environment:
        uid = node.get('uid')
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        if rec.get('description'):
            values['description'] = rec.attrib['description']
        ...
        from inphms.orm.fields import Command  # noqa: PLC0415
        groups = []
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler inphms.tools.convert:DEBUG" for complete traceback
    ...
    def __init__(self, env, module, idref: t.Optional[IdRef], mode: ConvertMode, noupdate: bool = False, xml_filename: str = ''):
        self.mode = mode
        self.module = module
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <inphms>, <inphms> or <data>."
        self._tag_root(de)
    
    DATA_ROOTS = ['inphms', 'data', 'inphms']

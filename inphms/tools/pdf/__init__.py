...
from inphms.tools.arabic_reshaper import reshape
from inphms.tools import parse_version, file_open, SENTINEL
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with Inphms's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Inphms's logo in the banner.
    ...
    with file_open('base/static/img/main_partner-image.png', mode='rb') as f:
        inphms_logo_file = io.BytesIO(f.read())
    inphms_logo = Image.open(inphms_logo_file)
    inphms_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(inphms_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class InphmsPdfFileReader(PdfFileReader):
...
class InphmsPdfFileWriter(PdfFileWriter):
    ...
    def embed_inphms_attachment(self, attachment, subtype=None):
        assert attachment, "embed_inphms_attachment cannot be called without attachment."

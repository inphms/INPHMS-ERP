...
from inphms.databases import table_exists
from inphms.tools import file_path, file_open
from .manifest import Manifest

if t.TYPE_CHECKING:
    from inphms.databases import Cursor, BaseCursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for info in Manifest.all_addon_manifests():
...
def has_trigram(cr: BaseCursor) -> bool:
    """ Test if the database has the a word_similarity function.
        ...
        pg_trgm module but any similar function will be picked by Inphms.

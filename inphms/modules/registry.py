...
from inphms import databases as sql_db
from inphms.tools import lazy_classproperty, LRU, locked, gc, Collector, OrderedSet, reset_cached_properties, format_frame
from inphms.config import config
from inphms.databases import SQL, _unaccent, remove_accents, sqlutils
from inphms.orm import model_classes
from .utils import _CACHES_BY_KEY, _REGISTRY_CACHES, SUPERUSER_ID, _REPLICA_RETRY_TIME
from .triggertree import TriggerTree

if t.TYPE_CHECKING:
    from inphms.orm.fields import Field
    from inphms.orm.models import BaseModel
    from inphms.databases import BaseCursor, Connection, Cursor
    from collections.abc import Collection, Callable, Iterable, Iterator
    from .graph import ModuleNode

_logger = logging.getLogger("inphms.registry")
...
class Registry(Mapping[str, type["BaseModel"]]):
    ...
    _saved_lock: threading.RLock | None = None
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from inphms.modules import db_init  # noqa: PLC0415
                if db_init.is_initialized(cr):
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            ...
            self._assertion_report: InphmsTestResult | None = InphmsTestResult()
        else:
            self._assertion_report = None

        self._ordinary_tables: set[str] | None = None                               # cached names of regular tables
        self._constraint_queue: dict[t.Any, Callable[[BaseCursor], None]] = {}      # queue of functions to call on finalization of constraints
        ...
        from . import db_init  # noqa: PLC0415
        with closing(self.cursor()) as cr:
            self.has_unaccent = db_init.has_unaccent(cr)
            self.has_trigram = db_init.has_trigram(cr)
    ...
    def descendants(self, model_names: Iterable[str], *kinds: t.Literal['_inherit', '_inherits']) -> OrderedSet[str]:
    ...
    def init_models(self, cr: Cursor, model_names: Iterable[str], context: dict[str, t.Any], install: bool = True):
    ...
    def load(self, module: ModuleNode) -> list[str]:
    ...
    ##############################################
    # Layer 1: Indexes, ForeignKeys, TableExists #
    ##############################################
    ...
    def check_indexes(self, cr: Cursor, model_names: Iterable[str]) -> None:
        ...
        expected = [
            (sqlutils.make_index_name(Model._table, field.name), Model._table, field)
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from .db_init import FunctionStatus  # noqa: PLC0415
                ...
                try:
                    with cr.savepoint(flush=False):
                        sqlutils.create_index(cr, indexname, tablename, [expression], method, where)
    ...
    def check_foreign_keys(self, cr: Cursor) -> None:
        ...
        for key, val in self._foreign_keys.items():
            ...
            deltype = sqlutils._CONFDELTYPES[ondelete.upper()]
            spec = existing.get(key)
            if spec is None:
                sqlutils.add_foreign_key(cr, table1, column1, table2, column2, ondelete)
                conname = sqlutils.get_foreign_keys(cr, table1, column1, table2, column2, ondelete)[0]
                ...
                sqlutils.drop_constraint(cr, table1, spec[0])
                sqlutils.add_foreign_key(cr, table1, column1, table2, column2, ondelete)
                conname = sqlutils.get_foreign_keys(cr, table1, column1, table2, column2, ondelete)[0]
                model.env['ir.model.constraint']._reflect_constraint(model, conname, 'f', None, module)
    ...
    def check_tables_exist(self, cr: Cursor) -> None:
        ...
        missing_tables = set(table2model).difference(sqlutils.existing_tables(cr, table2model))
    ...
    # region: CRUD of Models and Fields

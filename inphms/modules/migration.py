...
from inphms.tools import file_path, parse_version
...
if t.TYPE_CHECKING:
    ...
    from .graph import ModuleGraph, ModuleNode
...
VERSION_RE = re.compile(
    r"""^
        # Optional prefix with Inphms version
        ((
            6\.1|
        ...
        # After Inphms version we allow precisely 2 or 3 parts
...
class MigrationManager:
    ...
    def __init__(self, cr: Cursor, graph: ModuleGraph):
        self.cr = cr
        self.graph = graph
    ...
    def _get_files(self) -> None:
        def _get_upgrade_path(pkg: str) -> Iterator[str]:
            for path in inphms.upgrade.__path__:
    ...
    def migrate_module(self, pkg: ModuleNode, stage: t.Literal['pre', 'post', 'end']) -> None:
        ...
        def convert_version(version: str) -> str:
            ...
            return "%s.%s" % (release.MAJOR, version)
        ...
        def compare(version: str) -> bool:
            ...
            if majorless_version:
                # We should not re-execute major-less scripts when upgrading to new Inphms version

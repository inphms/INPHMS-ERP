...
from .module import setup_sys_path, load_inphms_module
from . import db_init
from .graph import ModuleGraph
from .utils import SUPERUSER_ID, adapt_version
from .migration import exec_script, MigrationManager
from .environments import Environment
from inphms.databases import column_exists, table_exists, Cursor, sql_counter, db_connect
from inphms.config import config
from inphms.tools.convertutils import convert_file, IdRef, ConvertMode as LoadMode
...
def load_modules(
    ...
    *,
    ...
    with registry.cursor() as cr:
        ...
        if not db_init.is_initialized(cr):
        ...
        if update_module and upgrade_modules:
            for pyfile in config['pre_upgrade_scripts']:
                exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')

        if update_module and table_exists(cr, 'ir_model_fields'):
        ...
        load_lang = config._cli_opts.pop('load_language', None)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['inphms.addons.%s' % (pkg.name,)]
...
def load_module_graph(
    ...
    assert isinstance(env.cr, Cursor), "Need for a real Cursor to load modules"
    ...
    t0 = time.time()
    loading_extra_query_count = sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = sql_counter
        ...
        load_inphms_module(package.name)

        if update_operation == 'install':
            py_module = sys.modules['inphms.addons.%s' % (module_name,)]
        ...
        update_from_config = config['update'] or config['init'] or config['reinit']
        if config['test_enable'] and (update_operation or not update_from_config):
            ...
            if suite.countTestCases():
                ...
                # Python tests
                tests_t0, tests_q0 = time.time(), sql_counter
                ...
                test_queries = sql_counter - tests_q0

                # tests may have reset the environment
                module = env['ir.module.module'].browse(module_id)

        extra_queries = sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_data(env: Environment, idref: IdRef, mode: LoadMode, kind: LoadKind, package: ModuleNode) -> bool:
    """n noupdate is False, unless it is demo data

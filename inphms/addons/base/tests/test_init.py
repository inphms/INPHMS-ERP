...
class TestInit(BaseCase):
    ...
    def inphms_modules_to_test(self):
        import inphms.cli  # noqa: PLC0415
        for path in (*inphms.__path__, *inphms.cli.__path__):
            parent = Path(path)
            for module in parent.iterdir():
                if (module.is_dir() or module.suffix == '.py') and '__' not in module.name:
                    if parent.name == 'inphms':
                        yield f"inphms.{module.stem}"
                    else:
                        yield f"odinphmsoo.{parent.name}.{module.stem}"
    ...
    def test_import(self):
        ...
        for module in sorted(self.inphms_modules_to_test()):

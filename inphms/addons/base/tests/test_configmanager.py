...
import inphms
from inphms.tests import TransactionCase
from inphms.tools import file_open, file_open_temporary_directory, file_path
from inphms.config import configmanager

EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = inphms.config.root_path.removesuffix('/inphms')
DEFAULT_DATADIR = inphms.config._default_opts['data_dir']

MISSING_HTTP_INTERFACE = """\
WARNING:inphms.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch.dict('os.environ', {'INPHMS_RC': EMPTY_CONFIG_PATH}, clear=True)
        patcher.start()
    ...
    def parse_reset(self, args=None):
        with (
            patch.dict(self.config._runtime_opts, {}),
            patch.dict(self.config._cli_opts, {}),
            patch.dict(self.config._env_opts, {}),
            patch.dict(self.config._file_opts, {}),
            patch.dict(self.config._default_opts, {}),
        ):
            cli = self.config._parse_config(args)
            return cli, dict(self.config.opts)

    def assertConfigEqual(self, truth):
        try:
            self.assertEqual(dict(self.config.opts), truth)
        except AssertionError as exc1:
            for k in set(self.config.opts).intersection(truth):
                try:
                    self.assertEqual(self.config.opts[k], truth[k], f"{k!r} doesn't match")
            ...
            if missing := set(self.config.opts).difference(truth):
                e = "missing from the test dict: " + ', '.join(missing)
                raise AssertionError(e) from exc1
            if missing := set(truth).difference(self.config.opts):
                e = "missing from the configuration: " + ', '.join(missing)
                raise AssertionError(e) from exc1
            raise

    def test_00_setUp(self):
        self.assertEqual(self.config.opts['config_file'], EMPTY_CONFIG_PATH)
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('inphms.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:inphms.config:option addons_path, no such directory '/tmp/inphms', skipped",
            "WARNING:inphms.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:inphms.config:option pre_upgrade_scripts, no such file '/tmp/pre-custom.py', skipped",
        ])
    ...
    def test_04_inphms16_config_file(self):
        # test that loading the Inphms 16.0 generated default config works
        ...
        with (
            ...
            self.assertLogs('inphms.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:inphms.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:inphms.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:inphms.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:inphms.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)

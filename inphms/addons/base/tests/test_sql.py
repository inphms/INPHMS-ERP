...
class TestSqlTools(TransactionCase):
    ...
    def test_add_constraint(self):
        ...
        sqlutils.add_constraint(self.env.cr, 'res_bank', 'test_constraint_dummy', definition)

        # ensure the constraint with % works and it's in the DB
        with self.assertRaises(CheckViolation), mute_logger('inphms.db'):
            self.env['res.bank'].create({'name': r'10% bank'})

        # ensure the definitions match
        db_definition = sqlutils.constraint_definition(self.env.cr, 'res_bank', 'test_constraint_dummy')
        self.assertEqual(db_definition, definition)

    def test_add_index(self):
        definition = "(name, id)"
        sqlutils.add_index(self.env.cr, 'res_bank_test_name', 'res_bank', definition, unique=False)

        # check the definition
        db_definition, db_comment = sqlutils.index_definition(self.env.cr, 'res_bank_test_name')
    ...
    def test_add_index_escape(self):
        ...
        sqlutils.add_index(self.env.cr, 'res_bank_test_percent_escape', 'res_bank', definition, unique=False, comment=comment)

        # ensure the definitions match (definition is the comment if it is set)
        db_definition, db_comment = sqlutils.index_definition(self.env.cr, 'res_bank_test_percent_escape')

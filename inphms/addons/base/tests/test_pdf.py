...
from inphms.tools import pdf, file_open
from inphms.tools.pdf import reshape_text
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.InphmsPdfFileReader(self.minimal_reader_buffer)

    def test_inphms_pdf_file_reader(self):
        ...
        r = pdf.InphmsPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_inphms_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.InphmsPdfFileWriter()
    ...
    def test_inphms_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.InphmsPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.InphmsPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.InphmsPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Inphms')
        self.assertEqual(pdf_info['/Creator'], 'Inphms')
        reader_buffer.close()

...
from inphms.addons.base.tests import mail_examples
from inphms.addons.base.tests.common import MockSmtplibCase
from inphms.tests import tagged, users
from inphms.tests.common import TransactionCase
...
class EmailConfigCase(TransactionCase):
    ...
    @patch.dict(config.opts, {"email_from": "settings@example.com"})
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    @mute_logger('inphms.models.unlink')
    def test_mail_server_priorities(self):
        """ Test if we choose the right mail server to send an email. Simulates
        simple Inphms DB so we have to spoof the FROM otherwise we cannot send
        any email. """
    ...
    @mute_logger('inphms.models.unlink', 'inphms.addons.base.models.ir_mail_server')
    ...
    def test_mail_server_config_bin(self):
        ...
        inphms-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the inphms-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from inphms-bin
    ...
    @mute_logger('inphms.models.unlink')
    @patch.dict(config.opts, {'from_filter': 'fake.com', 'smtp_server': 'cli_example.com'})
    def test_mail_server_config_cli(self):
        """ Test the mail server configuration when the "smtp_authentication" is
        "cli". It should take the configuration from the inphms-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the inphms-bin argument

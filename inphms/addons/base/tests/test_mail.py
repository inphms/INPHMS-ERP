...
from inphms.addons.base.models.ir_mail_server import extract_rfc2822_addresses
from inphms.addons.base.models.ir_qweb_fields import nl2br_enclose
from inphms.tests import tagged
from inphms.tests.common import BaseCase
from inphms.tools.mailutils import (
...
from inphms.tools import html_escape
...
<b>Subject:</b> test (#23)</p>
    ...
    def test_sanitize_unescape_emails(self):
        ...
        for not_email in not_emails:
            ...
            self.assertNotIn(html_escape(not_email), sanitized, 'html_sanitize stripped emails of original html')
   ...
   font-size: 30px; max-width: 100%; after: "Not sure
    ...
    def test_quote_blockquote(self):
        ...
        for ext in mail_examples.QUOTE_BLOCKQUOTE_OUT:
            self.assertIn(u'<span data-o-mail-quote="1">%s' % html_escape(ext), html)
    ...
    def test_quote_thunderbird(self):
        ...
        for ext in mail_examples.QUOTE_THUNDERBIRD_1_OUT:
            self.assertIn(u'<span data-o-mail-quote="1">%s</span>' % html_escape(ext), html)
    ...
    def test_quote_basic_text(self):
        ...
        for test, in_lst, out_lst in test_data:
            ...
            for text in in_lst:
                self.assertIn(text, new_html)
            for text in out_lst:
                self.assertIn(u'<span data-o-mail-quote="1">%s</span>' % html_escape(text), new_html)

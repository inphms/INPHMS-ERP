...
from inphms.tools import file_path, mute_logger
from inphms import config

from .common import TransactionCaseWithUserDemo
from inphms.addons.base.models.ir_mail_server import IrMail_Server
...
@patch('inphms.addons.base.models.ir_mail_server.SMTP_TIMEOUT', .1)
# prevent the CLI from interfering with the tests
@patch.dict(config.opts, {'smtp_server': ''})
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        # decrease aiosmtpd verbosity, inphms INFO = aiosmtpd WARNING
        ...
        # force aiosmtpd/inphms to bind/connect to a fixed ipv4 OR ipv6
        # address.
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        traffic in between the Inphms server and the legitimate SMTP
        server.
        """
        mail_server = self.env['ir.mail_server'].create({
            'name': 'test smtpd',

...
@inphms.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(inphms.tests.TransactionCase):
...
class TestReportsRenderingCommon(inphms.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        inphms.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@inphms.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@inphms.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
@inphms.tests.tagged('post_install', '-at_install')
class TestAggregatePdfReports(inphms.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()

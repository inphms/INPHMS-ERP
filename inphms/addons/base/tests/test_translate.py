...
from inphms.tools.translate import quote, unquote, xml_translate, html_translate, TranslationImporter, TranslationModuleReader
from inphms.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('inphms.tests.stats')
    ...
    def test_translate_xml_a(self):
        ...
        source = """<t t-name="stuff">
                        <ul class="nav navbar-nav">
                            <li class="nav-item">
                                <a class="nav-link oe_menu_leaf" href="/inphms/action-54?menu_id=42">
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('inphms.addons.base.models.ir_module.IrModuleModule._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslationExport(TransactionCase):
    ...
    def test_export_translatable_resources(self):
        ...
        with self.assertNoLogs('inphms.tools.translate', "ERROR"):
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Inphms'})
    #     country_1.with_context(lang='fr_FR').name = 'Inphms_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Inphms2'})
    #     with self.assertRaises(IntegrityError), mute_logger('inphms.db'):
    #         country_2.name = 'Inphms'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('inphms.db'):
    #         country_3 = Country.create({'name': 'Inphms'})
...
class TestTranslationTrigramIndexPatterns(BaseCase):
    def test_value_conversion(self):
        ...
        for value, expected, message in cases:
            self.assertEqual(sqlutils.value_to_translated_trigram_pattern(value), expected, message)
    ...
    def test_pattern_conversion(self):
        ...
        for original_pattern, escaped_pattern, message in cases:
            self.assertEqual(sqlutils.pattern_to_translated_trigram_pattern(original_pattern), escaped_pattern, message)

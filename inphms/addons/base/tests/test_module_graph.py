...
from inphms.modules.graph import ModuleGraph
from inphms.modules.manifest import Manifest
from inphms.modules.utils import _DEFAULT_MANIFEST
from inphms.tools import mute_logger


class TestGraph(BaseCase):
    @mute_logger('inphms.modules.graph')
    def _test_graph_order(
        ...
        with (
            patch('inphms.modules.graph.ModuleGraph._update_from_database'),
            patch('inphms.modules.graph.Manifest.for_addon', make_manifest),
            patch('inphms.modules.graph.ModuleGraph._imported_modules', {'studio_customization'}),
        ):
            dummy_cr = None

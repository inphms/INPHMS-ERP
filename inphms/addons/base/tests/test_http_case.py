...
from inphms.tests.common import HttpCase, tagged, ChromeBrowser, TEST_CURSOR_COOKIE_NAME, Like
...
class TestHttpCase(HttpCase):
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('inphms.tests.common.ChromeBrowser.take_screenshot', return_value=None):
    ...
    def test_console_log_object(self):
        logger = logging.getLogger('inphms')
        level = logger.level
...
class TestChromeBrowser(HttpCase):
    def setUp(self):
        ...
        with patch.dict(config.opts, {'screencasts': screencasts_dir, 'screenshots': config['screenshots']}):
...
class TestRequestRemainingNoCookie(TestRequestRemainingCommon):
    ...
    def test_requests_b(self):
        with self.assertLogs('inphms.tests.common') as log_catcher:
            self._test_requests_b()
        self.assertEqual(
            log_catcher.output,
            [Like('... inphms.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. '
...
class TestRequestRemainingStartDuringNext(TestRequestRemainingCommon):
    ...
    def test_requests_b(self):
        with self.assertLogs('inphms.tests.common') as log_catcher, self.allow_requests():
            self._test_requests_b()
        self.assertEqual(
            log_catcher.output,
            [Like('... inphms.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. '

...
from inphms import DATETIME_FORMATS_MAP, tools, config
...
from .utils import LangData, LangDataDict, intersperse
...
class ResLang(models.Model):
    ...
    def install_lang(self):
        ...
        This method is called from inphms/addons/base/data/res_lang_data.xml to load
        some language and set it as the default for every partners. The
        language is set via config by the '_initialize_db' method on the
        'db' object. This is a fragile solution and something else should be
        found.

        """
        # config['load_language'] is a comma-separated list or None
        lang_code = (config.get('load_language') or 'en_US').split(',')[0]
    ...
    def _get_code(self, code: str) -> str | t.Literal[False]:
    ...
    def write(self, vals):
        ...
        if vals.get('active') == False:
            ...
            if self.env['res.users'].with_context(active_test=False).search_count([('lang', 'in', lang_codes)], limit=1):
                raise UserError(_("You cannot archive the language in which Inphms was setup as it is used by automated processes."))

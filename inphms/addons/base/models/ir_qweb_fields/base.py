...
from inphms.tools import get_lang, _, babel_locale_parse, format_duration, float_is_zero, \
    guess_mimetype, format_date, floatutils
from inphms.tools.stringutils import format_decimalized_number
from inphms.tools.mailutils import safe_attrs
from inphms.orm import api, models, fields
from .utils import nl2br, TIMEDELTA_UNITS
...
class IrQwebFieldInteger(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        if options.get('format_decimalized_number'):
            return format_decimalized_number(value, options.get('precision_digits', 1))
...
class IrQwebFieldFloat(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        if precision is None:
            ...
            value = floatutils.float_round(value, precision_digits=precision)

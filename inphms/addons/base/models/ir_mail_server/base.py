...
from .utils import CertificateError, _logger, MailDeliveryException, _test_logger, \
    extract_rfc2822_addresses, SMTP_TIMEOUT, _verify_check_hostname_callback
from inphms.tools import email_domain_extract, email_domain_normalize, email_normalize, \
    _, unique, email_normalize_all, encapsulate_email, html2plaintext, human_size
from inphms.tools.mailutils import email_split_and_format, generate_tracking_message_id
from inphms import config
from inphms.exceptions import UserError
from inphms.modules import current_test
...
class IrMail_Server(models.Model):
    ...
    from_filter = fields.Char(
        ...
        help='Comma-separated list of addresses or domains for which this server can be used.\n'
             'e.g.: "notification@inphms.com" or "inphms.com"')
    ...
    def _disable_send(cls):
        ...
        return current_test or cls.pool._init
    ...
    def _connect__(self, host=None, port=None, user=None, password=None, encryption=None,  # noqa: PLW3201
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the inphms-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the inphms-bin argument "smtp_ssl_private_key"
        ...
        if mail_server and mail_server.smtp_authentication != "cli":
            ...
            smtp_server = host or config.get('smtp_server')
            smtp_port = config.get('smtp_port', 25) if port is None else port
            smtp_user = user or config.get('smtp_user')
            smtp_password = password or config.get('smtp_password')
            if mail_server:
                from_filter = mail_server.from_filter
            else:
            ...
            if smtp_encryption is None and config.get('smtp_ssl'):
                smtp_encryption = 'starttls' # smtp_ssl => STARTTLS as of v7
            smtp_ssl_certificate_filename = ssl_certificate or config.get('smtp_ssl_certificate_filename')
            smtp_ssl_private_key_filename = ssl_private_key or config.get('smtp_ssl_private_key_filename')
        ...
        # Store the "from_filter" of the mail server / inphms-bin argument to  know if we
    ...
    def _build_email__(self, email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,  # noqa: PLW3201
        ...
        if subtype == 'html' and not body_alternative:
            ...
            msg.add_alternative(html2plaintext(email_body), subtype='plain', charset='utf-8')
    ...
    def _get_default_from_filter(self):
        ...
        return self.env['ir.config_parameter'].sudo().get_param(
            'mail.default.from_filter', config.get('from_filter')
        )
    ...
    def _alter_message__(self, message, smtp_from, smtp_to_list):  # noqa: PLW3201
        ...
        # that do not impact SMTP To
            ...
            to = message['To'] or ''
            to_normalized = email_normalize_all(to)
            message.replace_header(
                'To', ', '.join([
                    to,
                    ', '.join(
                        address for address in email_split_and_format(x_msg_add_to)
                        if email_normalize(address, strict=False) not in to_normalized
                    ),
                ]
                ))
        ...
        del message['Bcc']                   # see inphms/inphms@2445f9e3c22db810d61996afde883e4ca608f15b
    ...
    def _prepare_smtp_to_list(self, message, smtp_session):
        ...
        return [
            ...
            for address in unique(extract_rfc2822_addresses(base))
            if (
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        :returns: A two-elements tuple: ``(Record<ir.mail_server>, email_from)``
          ...
          1. Mail server to use to send the email (``None`` if we use the inphms-bin arguments)
        ...
        # Skip if passed email_from is False (example Inphmsbot has no email address)
        ...
        # 5: SMTP config in inphms-bin arguments
        from_filter = self.env['ir.mail_server']._get_default_from_filter()

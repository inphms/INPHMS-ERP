...
from inphms.modules import Environment, SUPERUSER_ID, Registry
from inphms.tools import get_lang, submap, json_default
from inphms.exceptions import AccessDenied, AccessError, SessionExpiredException, MissingError
from .utils import EXTENSION_TO_WEB_MIMETYPES, request, ModelConverter, ModelsConverter, SignedIntConverter, \
    FasterRule
from inphms.server.utils import is_cors_preflight, SAFE_HTTP_METHODS, get_session_max_inactivity
from inphms.server.routing import ROUTING_KEYS, _generate_routing_rules
from inphms.server.response import Response
from inphms.server.http import root
from inphms import tools
from inphms.config import config
from inphms.tools.translate import code_translations
...
class IrHttp(models.AbstractModel):
    ...
    def _auth_method_none(cls):
        request.env = Environment(request.env.cr, None, request.env.context)
        ...
        try:
            if request.session.uid is not None:
                if not security.check_session(request.session, request.env, request):
                    ...
                    request.env = Environment(request.env.cr, None, request.session.context)
    ...
    def _pre_dispatch(cls, rule, args):
        ...
        for key, val in list(args.items()):
            ...
            try:
                ...
                if request.env.user.is_public or isinstance(e, MissingError):
    ...
    def _gc_sessions(self):
        if os.getenv("INPHMS_SKIP_GC_SESSIONS"):
            return

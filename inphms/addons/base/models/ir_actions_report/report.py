...
from inphms.tools import is_html_empty, _, check_barcode_encoding, createBarcodeDrawing, get_barcode_font, parse_version
from inphms.exceptions import UserError, RedirectWarning, AccessError
from inphms.config import config
from inphms.tools.pdf import PdfFileWriter, PdfFileReader, PdfReadError
from inphms.tools._vendor.safe_eval import safe_eval, time
from .utils import _wkhtml, _run_wkhtmltopdf, _split_table
...
class IrActionsReport(models.Model):
    ...
    def _run_wkhtmltoimage(self, bodies, width, height, image_format="jpg"):
        ...
        """
        if (current_test or config['test_enable']) and not self.env.context.get('force_image_rendering'):
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("Inphms is unable to merge the generated PDFs."))
    ...
    def _pre_render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if (current_test or config['test_enable']) and not self.env.context.get('force_report_rendering'):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('Inphms is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,

...
from inphms.exceptions import AccessDenied, UserError, ValidationError, AccessError
from inphms.server.utils import request, DEFAULT_LANG
from inphms.tools import email_domain_extract, is_html_empty, reset_cached_properties, frozendict, _
from inphms.modules import SUPERUSER_ID
from inphms.databases import SQL
from .utils import check_identity, MIN_ROUNDS, CryptContext
...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~inphms.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~inphms.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Inphms (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: inphms.exceptions.AccessDenied when old password is wrong
        :raise: inphms.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Inphms portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Inphms is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Inphms in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.inphms.com/documentation/latest/administration/install/deploy.html#https for details.",
                    source
                )

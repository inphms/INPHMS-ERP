...
from inphms.exceptions import AccessError, UserError, MissingError, ValidationError
...
from inphms.tools import consteq, limited_field_access_token, OrderedSet, _, str2bool, imageutils, human_size, \
    split_every
from inphms.tools.mimetypes import guess_mimetype, fix_filename_extension, _olecf_mimetypes
    ...
    def _postprocess_contents(self, values):
        ...
        if is_image_resizable and (values.get('datas') or values.get('raw')):
            ...
            if str2bool(max_resolution, True):
                try:
                    if is_raw:
                        img = imageutils.ImageProcess(values['raw'], verify_resolution=False)
                    else:  # datas
                        img = imageutils.ImageProcess(base64.b64decode(values['datas']), verify_resolution=False)

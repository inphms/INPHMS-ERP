...
from .utils import _logger, assert_log_admin_access, UserError, ACTION_DICT, request, _, MissingDependency, \
    STATES, MyWriter
from inphms.tools.translate import get_datafile_translation_path, get_po_paths, TranslationImporter
from inphms.modules import Manifest, Registry, current_test
from inphms.modules.utils import adapt_version
from inphms.orm.fields import Domain
from inphms import tools
from inphms.tools import topological_sort, parse_version, get_flag
from inphms.modules.db_init import create_categories
...
class IrModuleModule(models.Model):
    ...
    def get_module_info(cls, name):
        if isinstance(name, str):
            ...
            return Manifest.for_addon(name, display_warning=False) or {}
    ...
    def _get_latest_version(self):
        default_version = adapt_version('1.0')
        for module in self:
    ...
    license = fields.Selection([
        ...
        ('IEEL-1', 'Inphms Enterprise Edition License v1.0'),
        ('IPL-1', 'Inphms Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('Inphms Enterprise Module', default=False)
    ...
    def _button_immediate_function(self, function):
        ...
        if self.search_count([('state', 'in', ('to install', 'to upgrade', 'to remove'))], limit=1):
            raise UserError(_("Inphms is currently processing another module operation.\n"
        ...
        try:
            ...
            raise UserError(_("Inphms is currently processing a scheduled action.\n"
                              "Module operations are not possible at this time, "
                              "please try again later or contact your system administrator."))
        function(self)

        self.env.cr.commit()
        registry = Registry.new(self.env.cr.dbname, update_module=True)
        self.env.cr.commit()
    ...
    def button_uninstall(self):
        un_installable_modules = set(config['server_wide_modules']) & set(self.mapped('name'))
    ...
    def _update_category(self, category='Uncategorized'):
        ...
        if categs != current_category_path:
            cat_id = create_categories(self.env.cr, categs)
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == 'category_id':
            ...
            records = self.env['ir.module.category'].search_read(domain, ['display_name', 'submenu_icon'], order="sequence")

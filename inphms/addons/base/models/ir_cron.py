...
from inphms.orm.utils import GC_UNLINK_LIMIT
from inphms.databases import db_connect, SQL
from inphms.exceptions import UserError, LockError
from inphms.modules import Registry, Manifest, Environment
...
INPHMS_NOTIFY_FUNCTION = os.getenv('INPHMS_NOTIFY_FUNCTION', 'pg_notify')
...
class IrCron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('INPHMS_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cr, jobs):
        ...
        # reset_module_states.
        from inphms.modules.loading import reset_modules_state  # noqa: PLC0415
    ...
    def _run_job(cls, job) -> CompletionStatus:
        ...
        with cls.pool.cursor() as job_cr:
            ...
            env = Environment(job_cr, job['user_id'], {
    ...
    def write(self, vals):
        ...
        if ('nextcall' in vals or vals.get('active')) and os.getenv('INPHMS_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list: list[datetime]):
        ...
        if min(at_list) <= now or os.getenv('INPHMS_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
        return triggers
    ...
    def _notifydb(self):
        ...
        The INPHMS_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        IrCron modification and on trigger creation (regardless of call_at)
        """
        with db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(INPHMS_NOTIFY_FUNCTION), self.env.cr.dbname))

...
from inphms.databases import db_connect, SQL, sqlutils, close_db
from inphms.modules import Registry, Environment, SUPERUSER_ID, neutralize_database as _neutralize_database
from inphms.tools import exec_pg_environ, find_pg_tool, osutils, scan_languages
...
def check_super(passwd):
    if passwd and config.validate_master_password(passwd):
        return True
    raise AccessDenied()


def _check_faketime_mode(db_name):
    if os.getenv('INPHMS_FAKETIME_TEST_MODE') and db_name in config['db_name']:
        try:
    ...
    try:
        ...
        with db.cursor() as cr:
            ...
            if config['unaccent']:
                ...
                # But in the case of Inphms, we consider that those data don't
...
# This should be moved to inphms.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(registry.cursor()) as cr:
            ...
            if login:
                values['login'] = login
                emails = tools.email_split(login)
                if emails:
...
def dump_db(db_name, stream, backup_format='zip', with_filestore=True):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            ...
            if stream:
                osutils.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                osutils.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def exp_change_admin_password(new_password):
    config.set_master_password(new_password)
    config.save_config(['master_pwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True

#################################
# No - Master password required #
#################################
@tools.mute_logger('inphms.db')
...
def list_dbs(force=False) -> list[str]:
    if not config['db_manager'] and not force:
        raise AccessDenied()
    
    if config['db_list']:
        return sorted(config['db_list'])
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if sqlutils.table_exists(cr, 'ir_module_module'):
...
def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(config.root_path, 'addons/base/data/res_country_data.xml')).getroot()
